...
const getDefConfig = configFromStore(staticConfig);
...
if (cli.output) {
  runtimeConfig.output = cli.output;
}
    ...
    fs.createDirectorys(componentPath)
      .then(() => {
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          if (file === indexFile) {
            data = createIndex(name, runtimeConfig);
            promises.push(
              fs.writeFileAsync(
                filePath,
            ...
            promises.push(
              createReactComponent(componentName, runtimeConfig).then(template => fs.writeFileAsync(
                filePath,
                getConfig('typescript') ? template : formatPrettier(template, prettierParser, prettierOptions),
              )),
            );
          } else if (file.indexOf(`.spec.${ext}`) > -1 || file.indexOf(`.test.${ext}`) > -1) {
            if (!getConfig('notest')) {
              promises.push(
                createReactComponentTest(name, runtimeConfig).then(template => fs.writeFileAsync(
                  filePath,
                  getConfig('typescript') ? template : formatPrettier(template, prettierParser, prettierOptions),
                )),
              );
          ...
          if ((file.indexOf(`.stories.${ext}`) || file.indexOf(`.stories.${ext}`)) > -1) {
            promises.push(
              createReactComponentStories(name, runtimeConfig).then(template => fs.writeFileAsync(
                filePath,
                getConfig('typescript') ? template : formatPrettier(template, prettierParser, prettierOptions),
              )),
            );
...
function initialize() {
  ...
  const componentPath = path.join(ROOT_DIR, runtimeConfig.output, args[0]);
