...
config.mergeAll({
  flags: options.map(value => (cli[value.name] ? value.name : undefined)).filter(value => value),
  root: ROOT_DIR,
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          if (file === indexFile) {
            ...
            promises.push(
              fs.writeFileAsync(
                filePath,
                config.hasFlag('typescript') ? data : formatPrettier(data, prettierParser, prettierOptions),
              ),
            );
          } else if (file === `${name}.${ext}` || file === `${name}.${jsxExt}`) {
            promises.push(
              createReactComponent(componentName).then(template => fs.writeFileAsync(
                filePath,
                config.hasFlag('typescript') ? template : formatPrettier(template, prettierParser, prettierOptions),
              )),
            );
          } else if (file.indexOf(`.spec.${ext}`) > -1 || file.indexOf(`.test.${ext}`) > -1) {
            if (!config.hasFlag('notest')) {
              promises.push(
                createReactComponentTest(name).then(template => fs.writeFileAsync(
                  filePath,
                  config.hasFlag('typescript') ? template : formatPrettier(template, prettierParser, prettierOptions),
                )),
              );
          ...
          if ((file.indexOf(`.stories.${ext}`) || file.indexOf(`.stories.${ext}`)) > -1) {
            promises.push(
              createReactComponentStories(name).then(template => fs.writeFileAsync(
                filePath,
                config.hasFlag('typescript') ? template : formatPrettier(template, prettierParser, prettierOptions),
              )),
            );
