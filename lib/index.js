...
const {
  ...
  createTypeScriptFunctionalComponent,
...
program
  ...
  .option('-t, --typescript', 'Creates Typescript component and files')
      ...
      .then(() => {
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          if (file === indexFile) {
            ...
            const isFunctional = functional || (config && config.includes('functional'));

            if (isFunctional) {
              if (isTypeScript) {
                data = createTypeScriptFunctionalComponent(name);
              } else {
